...
class LiquidShaker:
    """
    Quản lý bình lắc theo thể tích (ml) và thuật toán trộn màu chất lỏng.

    Mô phỏng bình pha chế thực tế: mỗi lần rót sẽ cộng dồn thể tích và
    tính màu hỗn hợp theo trọng số thể tích (weighted average RGB).

    Attributes:
        capacity (float): Dung tích tối đa của bình (ml).
        contents (dict): Ánh xạ ``{tên_nguyên_liệu: số_ml}`` hiện có trong bình.
        current_volume (float): Tổng thể tích đang chứa (ml).
        mixed_color (tuple | None): Màu RGB hiện tại của hỗn hợp,
            ``None`` nếu bình rỗng.
    """
    def __init__(self, capacity=300.0):
        """
        Khởi tạo bình lắc rỗng với dung tích cho trước.

        Args:
            capacity (float): Dung tích tối đa (ml). Mặc định là ``300.0``.
        """
        self.capacity = capacity
        self.contents = {}        # {tên_nguyên_liệu: số_ml}
        self.current_volume = 0.0 # Tổng ml đang có
        self.mixed_color = None   # Màu RGB hiện tại của hỗn hợp

    def pour(self, item_name, amount, color):
        """
        Rót một lượng nguyên liệu vào bình và cập nhật màu hỗn hợp.

        Nếu bình đã đầy, trả về ``False`` ngay. Nếu lượng rót vượt quá
        dung tích còn lại, tự động cắt bớt để không tràn bình.

        Màu hỗn hợp được tính bằng trung bình có trọng số (weighted average)
        theo thể tích: màu mới được pha với màu cũ theo tỉ lệ thể tích tương ứng.

        Args:
            item_name (str): Tên nguyên liệu đang rót (vd: ``"Vodka"``).
            amount (float): Lượng rót (ml).
            color (tuple): Màu RGB của nguyên liệu, dạng ``(R, G, B)``.

        Returns:
            bool: ``True`` nếu rót thành công, ``False`` nếu bình đã đầy.
        """
        if self.current_volume >= self.capacity:
            return False # Bình đã đầy
            
        # Không cho rót tràn bình
        actual_amount = min(amount, self.capacity - self.current_volume)
        
        # Cộng dồn dung tích nguyên liệu
        self.contents[item_name] = self.contents.get(item_name, 0.0) + actual_amount
        
        # Cập nhật màu trộn theo tỉ lệ
        if self.mixed_color is None:
            self.mixed_color = color
        else:
            r1, g1, b1 = self.mixed_color
            r2, g2, b2 = color
            v1 = self.current_volume
            v2 = actual_amount
            total_v = v1 + v2
            
            self.mixed_color = (
                int((r1 * v1 + r2 * v2) / total_v),
                int((g1 * v1 + g2 * v2) / total_v),
                int((b1 * v1 + b2 * v2) / total_v)
            )
            
        self.current_volume += actual_amount
        return True

    def clear(self):
        """
        Đổ bỏ toàn bộ chất lỏng, đặt lại bình về trạng thái rỗng.

        Reset ``contents``, ``current_volume`` về 0 và ``mixed_color`` về ``None``.
        Được gọi khi người chơi nhấn phím C hoặc sau khi serve xong.
        """
        self.contents.clear()
    ...
    def get_proportions(self):
        """
        Tính tỉ lệ phần trăm thể tích của từng nguyên liệu trong bình.

        Được dùng bởi ``MixingEngine.check_and_serve()`` để so sánh
        với công thức chuẩn và tính độ lệch (margin of error).

        Returns:
            dict: ``{tên_nguyên_liệu: tỉ_lệ}`` với tổng các tỉ lệ = 1.0,
            hoặc ``{}`` nếu bình đang rỗng.
        """
        if self.current_volume == 0:
            return {}
        return {k: v / self.current_volume for k, v in self.contents.items()}
